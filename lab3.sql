...
--2 Download lab3.sql file from intranet.
-- Copy content of the file and run using Query Tool.
...
-- 4 Select the last name of all employees, without duplicates(distinct).
...
--12 Select the name of the department with the second highest budget.
SELECT name
FROM departments
...
--15 Select all from departments with a budget more than 60000,
-- ordered by increasing budget and decreasing code.
SELECT code, budget
FROM departments
WHERE budget > 60000
ORDER BY budget ASC, code DESC;

--16 Reduce the budget of the department with the lowest budget by 10%.
UPDATE departments
